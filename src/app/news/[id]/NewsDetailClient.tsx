...
export default function NewsDetailClient({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const filters = sessionStorage.getItem('news_last_filters')
        if (filters) {
    ...
    async function handleSubmitComment(e: React.FormEvent) {
        ...
        if (!limitCheck.canProceed) {
            alert(
                `Too many comments! Please wait ${limitCheck.resetIn} seconds. ` +
                `(Limit: ${RATE_LIMITS.COMMENT.maxAttempts} comments per minute)`
            );
            return;
        }

        const result = validate(commentSchema, { content: newComment })

        if (!result.success) {
            const firstError = Object.values(result.errors)[0]
            alert(firstError || 'Invalid comment')
            return
        }
    ...
    const handleBack = () => {
        if (lastFilters) {
            ...
            if (lastFilters.category !== "All") params.set('category', lastFilters.category)
        ...
        <main className={styles.main}>
            <div className={styles.container}>
                <nav className={styles.breadcrumbs} aria-label="Breadcrumb">
                    ...
                    <Link href="/news" className={styles.breadcrumb_link}>{t('news.title')}</Link>
                    <span className={styles.breadcrumb_separator}>/</span>
                    <span className={styles.breadcrumb_current}>{article.title}</span>
                </nav>

                <div className={styles.navigation_header}>
                    <button onClick={handleBack} className={`${styles.back_button} ripple`}>
                    ...
                    {lastFilters && (
                        <div className={styles.filter_context}>
                            ...
                            {lastFilters.category !== "All" && <span className={styles.filter_tag}>Category: {lastFilters.category}</span>}
                        </div>
                    )}
                </div>
